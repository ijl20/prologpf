...
/**************************************************************************/
/*    o_next(N, Accumulated_oracle_so_far)                      */
/*                                                                        */
/* will return the next oracle index in N (from head of global variable   */
/* o_orc) or will return leave N as new variable if o_orc = []            */
/* also length of oracle is tracked with o_depth, which is tested against */
...
o_bfp(G,N,L) :-        /* Phase 0: Collect oracles */
    o_init,
    ...
    o_set_limit(L),
      ...
      write(G), write(' '),
      write(L), nl,
    cpu_time(T0), g_assign(o_BFstart,T0),
    '$orc_0_o_query'(_,A,[_|A],_),
    fail.

o_bfp(G,N,L) :-       /* Phase 1: pick oracles and search */
    cpu_time(T1),
    % argv(1,G),
    % argv(2,N), 
    % argv(3,L), 
    ...
    BFtime is T1-T0, g_assign(o_BFtime,BFtime),
      ...
      write(S), write(' '),
      write(BFtime), nl,
    cpu_time(T2), g_assign(o_Ftime,T2),
    o_no_limit,
    o_get_oracle(G,N),
    print_orc,
    '$orc_0_o_query'(_,A,[Orc|A],_),
    fail.

o_bfp(G,N,L) :-      /* Phase 2: Shutdown */
    cpu_time(T3), g_read(o_Ftime, T2), Ftime is T3-T2,
    % argv(1,G),
    % argv(2,N), 
    % argv(3,L), 
    o_orc_count(S),
