...
function NodeStatus()
{
    ...
    // splitting.
    this.reserve_reset = function (reserved_count) {
        console_log('reserve_reset('+reserved_count+')');
        var reset_count = 0;
        for (var node_name in ppc_status)
        {
            if (ppc_status[node_name].status == PPC_RESERVED)
            {
                console_log('reserve_reset '+node_name);
...
function check_kappa()
{
    ...
    if (vars['KAPPA'] && waiting_count >= vars['KAPPA'])
    {
        ...
        // send 'split' command to a suitable ppc
        split_ppc();
    } else {
...
function split_assign(node_name, orc)
{
    console.log('---- Skynet split_assign',node_name, orc);
    // work will be assigned to KAPPA + 1 ppc's (to include the split ppc)
    skynet_bfp(node_name,'kappa',orc,parseInt(vars['KAPPA'])+1);
}
...
// (This is because this ppc was split)
function skynet_bfp(node_name, proc, O, G)
{
    console.log('---- Skynet calling skynet_bfp',node_name, proc,O,G);
    ...
    if (node_name)
    {
        ppc_set.push({ node_name: node_name, rnd: Math.random() })
        ppc_required--;
    }
    
    // accumulate the expected KAPPA node_names that are PPC_RESERVED into ppc_set
    ...
    for (var N = 0; N < G; N++)
    {
        ...
        ppc_send_msg(ppc_set[N].node_name, 'run ' + proc + ' ' + O + ' ' + G + ' ' + N + "\n");
    }
}
...
function handle_ppc_line(host_name, slot_name, msg)
{
    ...
    switch (words[1]) {
        ...
        case SPLIT:
            ...
            split_assign(node_name, words[2]);
            break;
...
net.createServer(function(socket){
    ...
    var host_name = "host_not_known";
    var slot_name = "slot_not_known";
