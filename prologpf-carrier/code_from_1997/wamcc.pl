...
:- main([wamcc0,wamcc1,wamcc2,wamcc3,wamcc4,
         wamcc5,wamcc6,wamcc7,wamcc8,
         wamcc_ocode,wamcc_ocode_pl,wamcc_kcode,f_utils,wamcc_fcode]).
...
/* ijl*/ atom_concat(Prefix,'.ppf',FilePPF),
...
set_default_options:-
	...
	g_assign(no_stack_test,f),
        ...
        g_assign(debug,        0),
        g_assign(ocode_pl,     f),   /* ijl */
        g_assign(kcode,        f),   /* ijl */
        g_assign(ocode,        f),   /* ijl */
        g_assign(fcode,        f),   /* ijl */
        g_assign(ppf,          f),   /* ijl */
        g_assign(verbose,      f).
...
traite_opt(ocode):-         g_assign(ocode,         t).  /* ijl */
traite_opt(kcode):-         g_assign(kcode,         t).  /* ijl */
traite_opt(ocode_pl):-      g_assign(ocode_pl,      t).  /* ijl */
traite_opt(fcode):-         g_assign(fcode,         t).  /* ijl */
traite_opt(ppf):-           g_assign(ppf,           t).  /* ijl */
...
display_help:-
	...
	formata('   -ppf           produce a .ppf file~n',[]),
	...
	formata('   -ocode_pl      compile with oracle support in Prolog~n',[]),
	formata('   -ocode         compile with oracle support in C~n',[]),
	formata('   -kcode         compile with kappa support ~n',[]),
	formata('   -fcode         compile with functions support~n',[]),
...
compiler(LSrc,Main):-
	g_read(mode_c,ModeC),
        /* ijl 2/jun/97  */ sucre_syntaxique(LSrc,LSrc1),  % cuts, ';' etc
        /* ijl 8/apr/97  */ prog_to_fcode(LSrc1,LSrc2),    % compile funcs
        /* ijl 19/may/97 */ prog_to_ocode_pl(LSrc2,LSrc3), % add ocode (Prolog)
        /* ijl 20/Aug/97 */ prog_to_ocode(LSrc3,LSrc4),    % add ocode (C)
        /* ijl 24/Oct/97 */ prog_to_kcode(LSrc4,LSrc5),    % add kappa code
        /* ijl 3/jun/97  */ sucre_syntaxique(LSrc5,LSrc6), % cleanup funcs 
        /* ijl 9/apr/97  */ create_ppf_file(LSrc6),        % output .ppf file
	exec_passes(LSrc6,LClComp),                                  !,
...
% ijl 2/6/97	sucre_syntaxique(ClSrc,LSrc,ClSrc1,LSrc1),                  !,
	format_interne(ClSrc,Arg1,T,C,NbChunk),                    !,
        /* ijl 7/apr/97 debug print_format_int(ClSrc,Arg1,T,C,NbChunk), */
	classif_variables(T,C),                                     !,
	generation_code(T,C,NbChunk,Pred,W),                        !,
        /* ijl 20/5/97 debug  print_gen_code(T,C,W), */
	allocation_varsX(W),                                        !,
...
create_ppf_file(Prog) :- 
       g_read(ppf,t), !,
       g_read(file_ppf, FilePPF), 
       tell(FilePPF),
       print_prog(Prog),
       told.
...
print_gen_code(Head, Body, WamCode) :-
    write(Head), write(' :- '), write(Body), nl,
    write('Translated to:'), nl,
    print_list(WamCode), nl.
