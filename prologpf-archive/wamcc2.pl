...
/* ijl 21/5/97 modified formate_lst_arg/5 to format VARS first */
...
formate_tete(T,DicoVar,T1,Arg1):- 
	...
	formate_pred_head(T,0,DicoVar,T1,_). % lier Arg1 si var /* ijl bugfix*/
...
formate_pred_head(Pred,NoPred,DicoVar,p(NoPred,F/N,LstArg1,_),InlinePred):-
	functor(Pred,F,N),
	Pred=..[_|LstArg],
	formate_lst_arg_head(LstArg,NoPred,DicoVar,f,LstArg1),
	(inline_predicate(F,N) -> InlinePred=t 
	                       ;  InlinePred=f).

formate_lst_arg_head([],_,_,_,[]).

formate_lst_arg_head([Arg|LstArg],NoPred,DicoVar,Stc,[Arg1|LstArg1]):-
	formate_arg_head(Arg,NoPred,DicoVar,Stc,Arg1),
	!,
	formate_lst_arg_head(LstArg,NoPred,DicoVar,Stc,LstArg1).


formate_lst_stc_args(LstArg,NoPred,DicoVar,Stc,LstArg1):-
        formate_lst_vars(LstArg,NoPred,DicoVar,Stc,LstArg1),
        formate_lst_nonvars(LstArg,NoPred,DicoVar,Stc,LstArg1).

/* utility relations used in bugfix version: */

/* 1: Convert variables at this level in list: */

formate_lst_vars([],_,_,_,[]).

formate_lst_vars([Var|LstArg],NoPred,DicoVar,Stc,[Var1|LstArg1]) :-
        var(Var),
        formate_arg_head(Var,NoPred,DicoVar,Stc,Var1),
        formate_lst_vars(LstArg,NoPred,DicoVar,Stc,LstArg1),
        !.

formate_lst_vars([_|LstArg],NoPred,DicoVar,Stc,[_|LstArg1]) :-
        formate_lst_vars(LstArg,NoPred,DicoVar,Stc,LstArg1).

/* 2: Convert non-variables at this level in list (& recurse): */

formate_lst_nonvars([],_,_,_,[]).

formate_lst_nonvars([Var|LstArg],NoPred,DicoVar,Stc,[_|LstArg1]) :-
        var(Var),
        formate_lst_nonvars(LstArg,NoPred,DicoVar,Stc,LstArg1),
        !.

formate_lst_nonvars([NonVar|LstArg],NoPred,DicoVar,Stc,[NonVar1|LstArg1]) :-
        formate_arg_head(NonVar,NoPred,DicoVar,Stc,NonVar1),
        formate_lst_nonvars(LstArg,NoPred,DicoVar,Stc,LstArg1).

formate_arg_head(X,NoPred,DicoVar,Stc,var(IGV,PremOcc,_)):-
	var(X),
...
formate_arg_head([],_,_,_,nil).

formate_arg_head([Car|Cdr],NoPred,DicoVar,_,lst(Car1,Cdr1)):-
	formate_arg_head(Car,NoPred,DicoVar,t,Car1),
	formate_arg_head(Cdr,NoPred,DicoVar,t,Cdr1).

formate_arg_head(Fonc,NoPred,DicoVar,_,Fonc1):-
	functor(Fonc,F,N),
	(N=0 -> type_atomic(F,Fonc1)
	     ;  (Fonc=..[_|LstArg],
	         formate_lst_stc_args(LstArg,NoPred,DicoVar,t,LstArg1),
