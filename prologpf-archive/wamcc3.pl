...
classif_tete(Pred,NoY):-
	classif_pred_head(Pred,NoY,_).   /* ijl 22/5/97 bugfix */
...
/*-------------------------------------------------------------------------*/
/*   ijl 22/5/97 bugfix to scan BREADTH-first for vars in structures       */

classif_pred_head(p(NoPred,_,LstArg,NoY1),NoY,NoY2):-
	classif_lst_arg_head(LstArg,NoPred,NoY,NoY1,[],LYAAlloc),
	alloc_vraies_varsY(LYAAlloc,NoY1,NoY2).

classif_lst_arg_head([],_,NoY,NoY,LYAAlloc,LYAAlloc).

classif_lst_arg_head([Arg|LstArg],NoPred,NoY,NoY2,LYAAlloc,LYAAlloc2):-
	classif_lst_arg_head(LstArg,NoPred,NoY,NoY1,LYAAlloc,LYAAlloc1),
	classif_arg_head(Arg,NoPred,NoY1,NoY2,LYAAlloc1,LYAAlloc2),
	!.

classif_lst_stc_args(LstArg,NoPred,NoY,NoY2,LYAAlloc,LYAAlloc2):-
        classif_lst_nonvars(LstArg,NoPred,NoY,NoY1,LYAAlloc,LYAAlloc1),
        classif_lst_vars(LstArg,NoPred,NoY1,NoY2,LYAAlloc1,LYAAlloc2),
	!.

/* classif_lst_args case 1:  scan through non-vars (handle these first) */

           /* end of list so complete */
classif_lst_nonvars([],_,NoY,NoY,LYAAlloc,LYAAlloc).

           /* arg is Var so skip & continue */
classif_lst_nonvars([var(_,_,_)|LstArg],NoPred,NoY,NoY2,LYAAlloc,LYAAlloc2) :-
        classif_lst_nonvars(LstArg,NoPred,NoY,NoY2,LYAAlloc,LYAAlloc2),
        !.

           /* arg is NonVar, so classify rest of list and then arg */
classif_lst_nonvars([Arg|LstArg],NoPred,NoY,NoY2,LYAAlloc,LYAAlloc2) :-
	classif_lst_nonvars(LstArg,NoPred,NoY,NoY1,LYAAlloc,LYAAlloc1),
	classif_arg_head(Arg,NoPred,NoY1,NoY2,LYAAlloc1,LYAAlloc2).

/* classif_lst_args case 2:  scan through vars */

           /* end of list so complete */
classif_lst_vars([],_,NoY,NoY,LYAAlloc,LYAAlloc).

           /* arg is Var, so classify rest of list and then arg */
classif_lst_vars([var(A,B,C)|LstArg],NoPred,NoY,NoY2,LYAAlloc,LYAAlloc2) :-
	classif_lst_vars(LstArg,NoPred,NoY,NoY1,LYAAlloc,LYAAlloc1),
	classif_arg_head(var(A,B,C),NoPred,NoY1,NoY2,LYAAlloc1,LYAAlloc2),
        !.

           /* arg is NonVar so skip & continue */
classif_lst_vars([_|LstArg],NoPred,NoY,NoY2,LYAAlloc,LYAAlloc2) :-
        classif_lst_vars(LstArg,NoPred,NoY,NoY2,LYAAlloc,LYAAlloc2).


classif_arg_head(var(igv(Nom,_,NoPred,_,_),_,t),NoPred,NoY,NoY,
       LYAAlloc,LYAAlloc):-
	var(Nom),
	Nom=x(_).                           % Unsafe inutilise si x(_)

classif_arg_head(var(igv(y(No),_,NoPDOcc,_,_),_,t),NoPred,NoY,NoY,
	LYAAlloc,[No|LYAAlloc]):-
	 var(NoPDOcc),                       % pour ne la mettre qu'une
	 NoPDOcc=NoPred.                     % fois en LYAAlloc 

classif_arg_head(var(_,_,f),_,NoY,NoY,LYAAlloc,LYAAlloc).

classif_arg_head(nil,       _,NoY,NoY,LYAAlloc,LYAAlloc).

classif_arg_head(cst(_),    _,NoY,NoY,LYAAlloc,LYAAlloc).

classif_arg_head(int(_),    _,NoY,NoY,LYAAlloc,LYAAlloc).

classif_arg_head(lst(Car,Cdr),NoPred,NoY,NoY1,LYAAlloc,LYAAlloc1):-
	classif_lst_stc_args([Car,Cdr],NoPred,NoY,NoY1,LYAAlloc,LYAAlloc1).

classif_arg_head(stc(_,LstArg),NoPred,NoY,NoY1,LYAAlloc,LYAAlloc1):-
	classif_lst_stc_args(LstArg,NoPred,NoY,NoY1,LYAAlloc,LYAAlloc1).

/*   ijl 22/5/97 end bugfix                                                */
/*-------------------------------------------------------------------------*/
