...
/************************************************************************* */
/* trivial 'complication' step: */

make_filename(Base,Ext,Fullname) :-
    atom_chars(Base,Bs),
    atom_chars(Ext,Es),
    append(Bs,Es,FullnameList),
    atom_chars(Fullname,FullnameList).


fmake(F) :- 
    make_filename(F,'.pl',FnIn),
    input_file(FnIn,FSrc),
    prog_to_fcode(FSrc,Src),
    make_filename(F,'_fcode.pl',FnOut),
    output_file(FnOut,Src).
    % assert_list(Src).
    
omake_pl(F) :- 
    make_filename(F,'.pl',FnIn),
    input_file(FnIn,FSrc),
    prog_to_ocode_pl(FSrc,Src),
    make_filename(F,'_ocode.pl',FnOut),
    output_file(FnOut,Src).
    % assert_list(Src).
