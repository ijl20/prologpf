...
// "flow" | "console"
var STATUS_CONSOLE = "console";
var STATUS_FLOW = "flow";
var status_format = STATUS_CONSOLE; // set status area to display in "flow" format
...
function user_set_status_flow()
{
    status_format = STATUS_FLOW;
    status_flow_init();
}

function user_set_status_console()
{
    status_format = STATUS_CONSOLE;
    status_console_init();
}
...
function status_console_init()
{
    if (status_format != STATUS_CONSOLE) return; // do nothing if status area not in "console" format
    status_area_init();
}
...
{
    if (status_format != STATUS_CONSOLE) return; // do nothing if status area not in "log" format
...
{
    if (status_format != STATUS_FLOW) return; // do nothing if status area not in STATUS_FLOW format
