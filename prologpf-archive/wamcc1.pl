...
/* ijl 2/6/97: sucre_syntaxique modified to scan whole prog, not just an   */
/* individual clause (previously was sucre_syntaxique1/4).                 */

:- public sucre_syntaxique/2.

sucre_syntaxique([],[]).

sucre_syntaxique([Cl|Prog],[Cl1|Prog2]) :-
        sucre_syntaxique1(Cl,Prog,Cl1,Prog1),
        sucre_syntaxique(Prog1,Prog2).

% code unchanged from here, except 'sucre_syntaxique' now 'sucre_syntaxique1'

sucre_syntaxique1(ClSrc,LSrc,ClSrc2,LSrc1):-
	(ClSrc=(T:-_) ; ClSrc=T),
        functor(T,F,N),
