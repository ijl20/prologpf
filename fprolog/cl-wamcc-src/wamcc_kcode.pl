/***************************************************************************/
/* transform program into ocode using KAPPA support                        */
/***************************************************************************/
/* prog_to_kcode(ProgIn, ProgOut) */
...
:- public prog_to_kcode/2.

/***************************************************************************/
/* transform program */

prog_to_kcode(Prog,Prog2) :- 
    g_read(kcode,t), !,              % transform if -kcode option set
...
prog_to_kcode(Prog,Prog).            % if no -ocode then no change
...
clause_to_ocode((H:-B), (H1:- (o_kbuild(N),B1))) :-
    functor(H,Func,A), mangle(Func,A,Func1),
...
clause_to_ocode(H, (H1 :- o_kbuild(N))) :-
    functor(H,Func,Arity), mangle(Func,Arity,Func1),
    proc_info_ocode(Func1),
    proc_info_next(Func1,N),
    H =.. [_|Args],
...
goal_to_ocode(P,(o_kfollow(N),P1)) :-
