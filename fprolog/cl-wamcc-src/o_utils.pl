...
:- public o_build/1, o_follow/1, o_soln/1, o_bfp_c/0, o_bfp_one_c/0.
...
o_bfp_one_c :-        /* Phase 0: Collect oracles */
    o_init,
    g_assign(o_SolnCount,0),
    /* Arg1 = G, Arg2 = N, Arg3 = L */
    argv(1,Arg1), number_atom(G,Arg1), o_set_g(G),
...
o_bfp_one_c :-       /* Phase 1: pick oracles and search */
    cputime(T1),
    argv(1, Arg1), number_atom(G,Arg1),
    argv(2, Arg2), number_atom(N,Arg2),
    argv(3, Arg3), number_atom(L,Arg3),
    ...
    o_set_follow,
    o_query,               /* FOLLOW mode query */
    o_bfp_one_c_complete.

o_bfp_one_c :-
    o_bfp_one_c_complete.

o_bfp_one_c_complete :-      /* Phase 2: Shutdown */
    cputime(T3), g_read(o_Ftime, T2), Ftime is T3-T2,
    argv(1, G),
    argv(2, N),
    argv(3, L),
