...
notthreatened(I, J, X, Y) :- I \== X, J \== Y, 
	...
	U2 is I + J, V2 is X + Y, U2 \== V2, !,g_read(cut_count,CC), CC1 is CC + 1, g_assign(cut_count,CC1).

safe(X, Y, []) :- !,g_read(cut_count,CC), CC1 is CC + 1, g_assign(cut_count,CC1).
safe(X, Y, [square(I, J) | L]) :-
	notthreatened(I, J, X, Y), safe(X, Y, L).

o_query :- get_solutions(8,X), o_soln(X).
