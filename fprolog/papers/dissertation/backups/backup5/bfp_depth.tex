\chapter{Pure Prolog with Breadth-First Partitioning} 
\label{bfp_depth}

\todo{name prior implementation: DelphiKS}
...
\todo{\cite{Sar95} also used benchmarks with little or-parallelism}
\todo{maybe swap this to after the queens, pent benchmarks}
...
\todo{maybe these shouldn't be subsections}
...
\todo{include depth and breadth info? total count of choice points?}

\todo{max or-only depth = 78 \cite{Kle91}}
...
\todo{depth, breadth of search tree? number of choice points?}

\todo{max or-only depth = 118 \cite{Kle91}}
...
\todo{max or-only depth = 323 \cite{Kle91}}
...
The previous implementation of the Delphi Machine \cite{Kle91,Sar95} included
...
The Prolog implementation of oracle support provides a flexible tool for
...
A query such as \texttt{:- a(X)} will initially create an oracle \texttt{[1]}, and then
...
\todo{add spaces to ytics in single cpu graphs}
\begin{figure}[htbp]
...
\todo{improve first list item.  slope less than one can still mean halving}
\begin{itemize}
...
Table \ref{q8_orc_counts} also contains some calculated values for the minumum execution
...
\begin{figure}[p]
\vspace{5mm} \hbox to \hsize{\hfill \psfig{file={bfp_depth/ps/blank.ps}} \hfill}
...
If further refinement is required, the subtree referred to by oracle Cx (see above) can
