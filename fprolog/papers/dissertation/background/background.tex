...
A subgoal \texttt{r(U,V)} appears in each dashed area, the first of which 
...
Concurrent Prolog does not require the guard literals to be ground at
...
This meta-logical information enables the compiler to perform significant additional error
...
\>\>$\Downarrow$\\
\pagebreak                      % manual page break for final formatting
\\
\>(X=Nil and Y=[a,b]) or\\
\>SOME [Z] (X=[a|Z] and ((Z=Nil and Y=[b]) or\\
...
Some scheduling strategies such as breadth-first partitioning, described in \cite{Sar95} 
...
\end{figure}
  ...
  \item[Partition left.]{ This strategy is the same as \textit{partition right} given above,
    ...
    left-most, with the
    remaining \mbox{$G-(M-1)$} all taking the left-most branch.}
...
\item{\textbf{Reassign-job.}  This scheduling strategy was proposed by Klein in
  \cite{Kle91}, with further analysis by Saraswat in \cite{Sar95}.
  ...
  idle processors including
  the processor which reported the oracle, with the reported oracle defining
...
\item{\textbf{Breadth-first partitioning.} This strategy is suggested by Alshawi and Moran
  in \cite{AM88}, and was implemented as an extension to DelphiKS by Saraswat \cite{Sar95}.
  The strategy greatly improves the granularity of task assignment over the earlier
  strategies, and is used by PrologPF.
  ...
  the work in the initial distribution, and the breadth-first partitioning strategy achieves
  this.
