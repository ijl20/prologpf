...
perm(Xs,[X|Ys]) :- o_build(1), sel(X,Xs,Xs1), perm(Xs1,Ys).
perm([],[]):- o_build(2).

sel(X,[X|Xs],Xs) :- o_build(1).
sel(X,[X1|Xs],[X1|Ys]) :- o_build(2), sel(X,Xs,Ys).

% follow code

perm(0,A,B) :- perm(A,B).
perm(1,Xs,[X|Ys]) :- o_follow(N1), sel(N1,X,Xs,Xs1),
                     o_follow(N2), perm(N2,Xs1,Ys).
perm(2,[],[]).

sel(0,A,B,C) :- sel(A,B,C).
sel(1,X,[X|Xs],Xs).
sel(2,X,[X1|Xs],[X1|Ys]) :- o_follow(N1), sel(N1,X,Xs,Ys).

o_query :- true.

b_query :- perm([1,2,3,4,5,6],X), o_soln(X).

f_query :- o_follow(N), perm(N,[1,2,3,4,5,6],X), o_soln(X).
